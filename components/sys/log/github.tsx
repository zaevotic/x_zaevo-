...
export default function GithubHeatmap() {
  ...
  useEffect(() => {
    fetch("/api/sys/github")
      ...
      .then((payload) => {
        ...
        payload.weeks.forEach(
          (week: { contributionDays: { contributionCount: number }[] }) => {
            week.contributionDays.forEach(({ contributionCount: c }) => {
              counts.push(c);
              cells.push(
                c === 0
                  ? ""
        ...
        for (const c of counts) {
          currentStreak = c > 0 ? currentStreak + 1 : 0;
          if (currentStreak > maxStreak) maxStreak = currentStreak;
        }

        setData({ cells, counts, commits: payload.total, streak: maxStreak });
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="w-full relative mt-[8px]">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mt-[14px] gap-[12px]">
        {/* Legend */}
        <div
        ...
        >
          ...
          <div
            ...
            style={{ background: LEVEL_COLORS[""] }}
          />
          <div
            className="w-[10px] h-[10px] rounded-[2px]"
            style={{ background: LEVEL_COLORS.l1 }}
          />
          <div
            className="w-[10px] h-[10px] rounded-[2px]"
            style={{ background: LEVEL_COLORS.l2 }}
          />
          <div
            className="w-[10px] h-[10px] rounded-[2px]"
            style={{ background: LEVEL_COLORS.l3 }}
          />
          <div
            className="w-[10px] h-[10px] rounded-[2px]"
            style={{
              background: LEVEL_COLORS.l4,
              outline: "1px solid var(--red-ember)",
              outlineOffset: -1,
            }}
          />
          <span>More</span>
        ...
        <div className="flex items-center gap-[12px] text-[10px]">
          <span style={{ color: "var(--text2)" }}>
            <b style={{ color: "var(--red-ember)" }}>{data.commits}</b>{" "}
            contributions this year
          </span>
          <span style={{ color: "var(--border2)" }}>·</span>
          <span style={{ color: "var(--text2)" }}>
            <b style={{ color: "var(--red-ember)" }}>{streakLabel}</b>{" "}
            streak
          </span>
        </div>
      </div>
