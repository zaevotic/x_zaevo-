...
export default function KeyboardPanel() {
  ...
  const keyRows = [
    [
      ...
      { code: 'F1', label: 'F1', kind: 'fn' }, { code: 'F2', label: 'F2', kind: 'fn' }, { code: 'F3', label: 'F3', kind: 'fn' }, { code: 'F4', label: 'F4', kind: 'fn' },
      { code: 'F5', label: 'F5', kind: 'fn' }, { code: 'F6', label: 'F6', kind: 'fn' }, { code: 'F7', label: 'F7', kind: 'fn' }, { code: 'F8', label: 'F8', kind: 'fn' },
      { code: 'F9', label: 'F9', kind: 'fn' }, { code: 'F10', label: 'F10', kind: 'fn' }, { code: 'F11', label: 'F11', kind: 'fn' }, { code: 'F12', label: 'F12', kind: 'fn' },
    ],
    [
      ...
      { code: 'Digit8', label: '8' }, { code: 'Digit9', label: '9' }, { code: 'Digit0', label: '0' },
      { code: 'Minus', label: '-' }, { code: 'Equal', label: '=' }, { code: 'Backspace', label: '⌫', kind: 'wide' },
    ],
    [
    ...
    [
      ...
      { code: 'KeyL', label: 'l' }, { code: 'Semicolon', label: ';' }, { code: 'Quote', label: "'\''" }, { code: 'Enter', label: '↵', kind: 'wider' },
    ],
    [
      ...
      { code: 'Comma', label: ',' }, { code: 'Period', label: '.' }, { code: 'Slash', label: '/' }, { code: 'ShiftRight', label: '⇧', kind: 'wider' },
    ],
    [
  ...
  const getKeyClasses = (key: any) => {
    let classes = "flex items-center justify-center border border-b-2 rounded-[3px] text-[9px] whitespace-nowrap select-none transition-all duration-75 ";
    ...
    if (activeCodes.has(key.code)) {
      classes += "bg-[var(--red-mute)] border-[var(--red-dim)] border-b-[var(--red-dim)] text-[var(--red)] translate-y-[2px] border-b-0";
    } else {
      classes += "bg-[var(--bg2)] border-[var(--border2)] border-b-[var(--border)] text-[var(--text3)]";
    }
  ...
  return (
    <div className="flex flex-col h-full items-center justify-center">
      ...
      <p className="text-[9px] tracking-[.1em] text-[var(--text3)] mt-[10px]">LIVE INPUT FEED</p>
    </div>
  );
}
